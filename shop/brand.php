...
-->
	...
	<?php 
	$sql=$mysqli->query("SELECT brandname FROM brand WHERE brandid=$brand_id");
	if($sql){
		while($obj = $sql->fetch_object()) {
			echo '<h3>'.$obj->brandname.' Brand</h3>';
		}
	} ?><hr class="soften" />
</div>
		  ...
		  <ul class="thumbnails">
		  	<?php
          $sql = $mysqli->query("SELECT * FROM product WHERE brandid=$brand_id and Status ='InStock'");
          if($sql){
              while($obj = $sql->fetch_object()) : 
             echo '<li class="span3">
			  <div class="thumbnail" >
