...
-->
	...
	<?php 
	$sql=$mysqli->query("SELECT genre FROM gen WHERE genid=$gen_id");
	if($sql){
		while($obj = $sql->fetch_object()) {
			echo '<h3>'.$obj->genre.'</h3>';
		}
	} ?>
	<hr class="soften"/>
	    </div>
    ...
    <ul class="thumbnails">
		  	<?php
          $sql = $mysqli->query("SELECT * FROM product WHERE Status='InStock' AND genid=$gen_id");
          if($sql){
              while($obj = $sql->fetch_object()) : 
             echo '<li class="span3">
			  <div class="thumbnail" >
